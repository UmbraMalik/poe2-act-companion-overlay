...
test('main process keeps renderer windows isolated and guards shell.openExternal', () => {
  ...
  assert.match(readText('src/shared/external-url-policy.ts'), /PROJECT_DONATION_ALERTS_URL/);
  assert.match(readText('src/shared/external-url-policy.ts'), /parsed\.protocol !== 'https:'/);
  assert.doesNotMatch(readText('src/main/app-environment.ts'), /parsed\.protocol === 'https:' \|\| parsed\.protocol === 'http:'/);
